...
export default function DealDetailPage({
  ...
  const handleDeleteDeal = async () => {
    if (!confirm("Are you sure you want to delete this deal? This will permanently delete the deal and reverse the capital ledger entries.")) return;
    ...
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <Link href="/deals" className="p-2 rounded-lg hover:bg-indigo-500/10 text-[#64748b] hover:text-indigo-400 transition-colors shrink-0">
        ...
        <div className="flex items-center gap-2 pl-12 sm:pl-0">
          <Link 
            href={`/deals/new?ipoName=${encodeURIComponent(deal.ipoName)}&applyDate=${deal.applyDate.split('T')[0]}&lots=${deal.lots || ""}&fundingType=${deal.fundingType}${deal.allotmentDate ? `&allotmentDate=${deal.allotmentDate.split('T')[0]}` : ""}${deal.listingDate ? `&listingDate=${deal.listingDate.split('T')[0]}` : ""}`}
            className="px-3 py-1.5 rounded-lg bg-indigo-500/10 text-indigo-400 text-xs font-medium hover:bg-indigo-500/20 transition-colors flex items-center gap-1.5"
          >
          ...
          <button 
            ...
            className="px-3 py-1.5 rounded-lg bg-rose-500/10 text-rose-400 text-xs font-medium hover:bg-rose-500/20 transition-colors flex items-center gap-1.5 disabled:opacity-50"
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
